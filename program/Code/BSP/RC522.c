...
uint8_t RC522_Read_Register( uint8_t Address )
{
	...
	data = RC522_SPI_ReadByte(0x00);//读取寄存器中的值
	RC522_CS_Disable();
	return data;
}
...
{
	RC522_Reset_Disable();
	bsp_Delayus ( 100 );	//真是复位时间太短问题！
	RC522_Reset_Enable();
	bsp_Delayus ( 100 );
	RC522_Reset_Disable();
	bsp_Delayus ( 100 );
	RC522_Write_Register( CommandReg, 0x0F );//复位
}
...
{
		RC522_ClearBit_Register( Status2Reg, 0x08 );//包含接收器、发射器和数据模式检测器的状态位。
		RC522_Write_Register( ModeReg, 0x3D );//定义发送和接收常用模式
		RC522_Write_Register( RxSelReg, 0x86 );//选择内部的接收器设置
		...
		RC522_Write_Register( TxAutoReg, 0x40 );     //控制天线驱动器的设置
		HAL_Delay(100);
		/* 开天线 */
		RC522_Antenna_On();
}
...
void CalulateCRC ( uint8_t * pIndata, uint8_t ucLen, uint8_t * pOutData )
{
    ...
    do 
    {
        ucN = RC522_Read_Register ( DivIrqReg );	//等待RC522计算
        uc --;
    } while ( ( uc != 0 ) && ! ( ucN & 0x04 ) );
...
//    printf("PcdSelect: Status=0x%02X, Len=0x%02X\r\n", ucN, ulLen); // <--- 添加这行
